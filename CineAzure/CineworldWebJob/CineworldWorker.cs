...
using System.IO;
using CineworldWebJob.MobileService;

namespace CineworldWebJob
{
    public class CineworldWorker
    {
        ...
        public void Run()
        {
            ...
            Console.WriteLine("Cineworld WorkerRole starting");
            ...
            Console.WriteLine("Populate location data");
            ...
            if (DateTime.UtcNow >= dtNextFullRun)
            {
                ...
                Console.WriteLine("Get TMDB Config");
                ...
                while (true)
                {
                    try
                    {
                        if (!processedUKFilms)
                        {
                            Console.WriteLine("Process Cineworld UK Films");
                        ...
                        {
                            Console.WriteLine("Process Cineworld UK Cinemas");
                        ...
                        if (!processedIEFilms)
                        {
                            Console.WriteLine("Process Cineworld Ireland Films");
                        ...
                        {
                            Console.WriteLine("Process Cineworld Ireland Cinemas");
                        ...
                        if (!cinemafilmSaved)
                        {
                            Console.WriteLine("Save Film / Cimema Lists");

                            ProcessData();
                            cinemafilmSaved = true;
                        }

                        Console.WriteLine("Process Performance info");
        ...
        private void LoadLocationData()
        {
            ...
            this.LongitudeDictionary.Add(91, -0.265683);
            #endregion
            ...
            this.LatitudeDictionary.Add(91, 52.228699);
        ...
        private void LogError(Exception ex, string message)
        {
            ...
            Microsoft.WindowsAzure.Storage.CloudStorageAccount storageAccount = Microsoft.WindowsAzure.Storage.CloudStorageAccount.Parse(ConfigurationManager.ConnectionStrings["AzureWebJobsStorage"].ConnectionString);

            Microsoft.WindowsAzure.Storage.Blob.CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();

            // Retrieve a reference to a container. 
            Microsoft.WindowsAzure.Storage.Blob.CloudBlobContainer container = blobClient.GetContainerReference("cineworld");
        ...
        private void ProcessCinemaInfo(Dictionary<int, CinemaInfo> currentCinemas, Dictionary<int, List<FilmInfo>> cinemaFilms, Dictionary<int, FilmInfo> Films, RegionDef region)
        {
            ...
            Console.WriteLine("Cinema list received from Cineworld");
            ...
            foreach (var cinema in cinemas.cinemas)
            {
                ...
                if (!currentCinemas.ContainsKey(cinema.id))
                {
                    ...
                    {
                        Console.WriteLine("Execute a Bing Query to retrieve coordinate");
                        ...
                        Console.WriteLine("Address: {0} {1}", cinema.address, cinema.postcode);
                    ...
                    Console.WriteLine("Cinema {0} reviews received", ci.Name);
                ...
                Console.WriteLine("Films for Cinema {0} received from cineworld", cinema.Name);
        ...
        {
            List<Task> PerformanceTaskList = new List<Task>();
            foreach (var cinemafilms in this.cinemaFilmsUK)
            {
                ...
                try
                {
                    PerformanceTaskList.Add(ProcessCinemaPerformances(RegionDef.GB, cinemafilms.Key, cinemafilms.Value));
                }
                catch (Exception ex)
                {
            ...
            foreach (var cinemafilms in this.cinemaFilmsIE)
            {
                try
                {
                    PerformanceTaskList.Add(ProcessCinemaPerformances(RegionDef.IE, cinemafilms.Key, cinemafilms.Value));
                }
                catch (Exception ex)
                {
            ...
            if(PerformanceTaskList.Count > 0)
            {
                //Task t = Task.WhenAll(PerformanceTaskList);
                ...
                //}
                Task.WaitAll(PerformanceTaskList.ToArray());
            }
        }

        private async Task ProcessCinemaPerformances(RegionDef region, int cinemaID, List<FilmInfo> films)
        {
            await Task.Run(() =>
            {
                Console.WriteLine("Process performances for cinema {0}", cinemaID);
                ...
                foreach (var film in films)
                {
                    try
                    {
                        Console.WriteLine("Get Dates Cinema {0}, Film {1}", cinemaID, film.Title);
                        Task<Dates> tdates = cws.GetDates(region, cinemaID, film.EDI);
                        tdates.Wait();

                        Console.WriteLine("Received Dates Cinema {0}, Film {1}", cinemaID, film.Title);

                        HashSet<DateTime> perfDates = new HashSet<DateTime>();
                        if (tdates.IsCompleted && tdates.Result != null)
                        {
                            ...
                            foreach (var date in tdates.Result.dates)
                            {
                                DateTime performanceDate = DateTime.ParseExact(date, "yyyyMMdd", CultureInfo.InvariantCulture);

                                Console.WriteLine("Get Performances Cinema {0}, Film {1}, Date {2}", cinemaID, film.Title, performanceDate);
                                Task<Performances> tp = cws.GetPerformances(region, cinemaID, film.EDI, date);
                                tp.Wait();
                                Console.WriteLine("Received Performances Cinema {0}, Film {1}, Date {2}", cinemaID, film.Title, performanceDate);

                                if (tp.IsCompleted && tp.Result != null)
                                {
        ...
        private void ProcessFilmInfo(Cineworld.Configuration config, Dictionary<int, FilmInfo> currentFilms, Dictionary<int, List<CinemaInfo>> filmCinemas, RegionDef region, bool searchTMDB = true)
        {
            ...
            Console.WriteLine("Film list received from Cineworld");
                ...
                if (!currentFilms.ContainsKey(film.edi) || !currentFilms[film.edi].TMDBDataLoaded)
                {
                    ...
                    if (movieData == null)
                    {
                        ...
                        try
                        {
                            ...
                            Console.WriteLine("TMDB search for film complete");
                    ...
                    Console.WriteLine("Film reviews received");
